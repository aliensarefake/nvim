...
--------------------------------------------------------------------------------
-- TODO HIGHLIGHTING (Custom, no plugin)
--------------------------------------------------------------------------------
-- Highlights TODO:, NOTE:, FIX: in comments and markdown
-- Supports: # (python/shell), // (c/js), -- (lua), bare text (markdown)

-- Define highlight groups (vibrant badge-style like todo-comments plugin)
local function setup_todo_highlights()
  vim.api.nvim_set_hl(0, "CustomTodo", { fg = "#000000", bg = "#0db9d7", bold = true })       -- black on cyan
  vim.api.nvim_set_hl(0, "CustomNote", { fg = "#000000", bg = "#10b981", bold = true })       -- black on green
  vim.api.nvim_set_hl(0, "CustomFix", { fg = "#000000", bg = "#ff5f5f", bold = true })        -- black on red
  vim.api.nvim_set_hl(0, "CustomImportant", { fg = "#000000", bg = "#ff9e64", bold = true })  -- black on orange
end

-- Pattern for each keyword (matches keyword + colon + rest of line)
-- Works with: # TODO: ..., // TODO: ..., -- TODO: ..., or just TODO: ...
local todo_patterns = {
  { pattern = [[\v(#|//|--|\*|)?\s*\zsTODO:.*]], group = "CustomTodo" },
  { pattern = [[\v(#|//|--|\*|)?\s*\zsNOTE:.*]], group = "CustomNote" },
  { pattern = [[\v(#|//|--|\*|)?\s*\zsFIX:.*]], group = "CustomFix" },
  { pattern = [[\v(#|//|--|\*|)?\s*\zsFIXME:.*]], group = "CustomFix" },
  { pattern = [[\v(#|//|--|\*|)?\s*\zsBUG:.*]], group = "CustomFix" },
  { pattern = [[\v(#|//|--|\*|)?\s*\zsCARELESS:.*]], group = "CustomFix" },
  { pattern = [[\v(#|//|--|\*|)?\s*\zsIMPORTANT:.*]], group = "CustomImportant" },
}

-- Apply highlights to current buffer
function M.apply_todo_highlights()
  -- Clear existing matches for this buffer
  vim.fn.clearmatches()

  -- Add match for each pattern
  for _, item in ipairs(todo_patterns) do
    vim.fn.matchadd(item.group, item.pattern)
  end
end

-- Jump to next/prev TODO in buffer
function M.jump_todo(direction)
  local patterns = "TODO:\\|NOTE:\\|FIX:\\|FIXME:\\|BUG:\\|CARELESS:\\|IMPORTANT:"
  local flags = direction == "next" and "w" or "bw"
  local found = vim.fn.search(patterns, flags)
  if found == 0 then
    vim.notify("No more TODOs", vim.log.levels.INFO)
  end
end

-- List all TODOs in current buffer (quickfix)
function M.list_todos_buffer()
  ...
  for i, line in ipairs(lines) do
    if line:match("TODO:") or line:match("NOTE:") or line:match("FIX:") or line:match("FIXME:") or line:match("BUG:") or line:match("CARELESS:") or line:match("IMPORTANT:") then
      table.insert(todos, {
        bufnr = bufnr,
        filename = filename,
        lnum = i,
        text = line:gsub("^%s+", ""),  -- trim leading whitespace
      })
    end
  end

  if #todos == 0 then
    vim.notify("No TODOs in buffer", vim.log.levels.INFO)
    return
  end
...
-- Search TODOs across project (uses grep)
function M.list_todos_project()
  vim.cmd([[silent! grep! "TODO:\|NOTE:\|FIX:\|FIXME:\|BUG:\|CARELESS:\|IMPORTANT:" . -r]])
  vim.cmd("copen")
end
...
function M.setup()
  ...
  -- Apply TODO highlights on buffer enter
  vim.api.nvim_create_autocmd({ "BufEnter", "BufWinEnter" }, {
    callback = M.apply_todo_highlights,
  })

  -- TODO keymaps
  vim.keymap.set("n", "]t", function() M.jump_todo("next") end, { desc = "Next TODO" })
  vim.keymap.set("n", "[t", function() M.jump_todo("prev") end, { desc = "Previous TODO" })
  vim.keymap.set("n", "<leader>ft", M.list_todos_buffer, { desc = "List TODOs in buffer" })
  vim.keymap.set("n", "<leader>fT", M.list_todos_project, { desc = "List TODOs in project" })
